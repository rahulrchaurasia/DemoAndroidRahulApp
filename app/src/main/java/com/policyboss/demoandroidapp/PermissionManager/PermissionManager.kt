...
class PermissionManager private constructor(private val fragment: WeakReference<Fragment>) {
    ...
    fun redirectAppSetting(fragment: Fragment) {
        androidx.appcompat.app.AlertDialog.Builder(fragment.requireContext(), R.style.ThemeOverlay_App_MaterialAlertDialog)
            .setTitle("App Setting")
            .setMessage(
                rationale
                    ?: "SevenPay application required permission, Please grant permission from application setting."
            )
            .setCancelable(true)
